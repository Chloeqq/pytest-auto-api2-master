...
TestData = CaseData(ConfigHandler.data_path + r'JkMhy/jkmhy_paysucess.yaml').case_process()
re_data = regular(str(TestData))


@allure.epic("开发平台接口")
@allure.feature("健客推荐模块")
class TestJkmhyPaysucess:

    @allure.story("物流页-可能帮到你")
    @pytest.mark.parametrize('in_data', eval(re_data), ids=[i['detail'] for i in TestData])
    def test_jkmhy_paysucess(self, in_data, case_skip):
        """
        :param :
        :return:
        """
...
if __name__ == '__main__':
    pytest.main(['test_jkmhy_paysucess.py', '-s', '-W', 'ignore:Module already imported:pytest.PytestWarning'])
