...
TestData = CaseData(ConfigHandler.data_path + r'JkUserCenter/jkusercenter.yaml').case_process()
re_data = regular(str(TestData))


@allure.epic("开发平台接口")
@allure.feature("个人中心模块")
class TestJkusercenter:

    @allure.story("个人中心页")
    @pytest.mark.parametrize('in_data', eval(re_data), ids=[i['detail'] for i in TestData])
    def test_jkusercenter(self, in_data, case_skip):
        """
        :param :
        :return:
        """
...
if __name__ == '__main__':
    pytest.main(['test_jkusercenter.py', '-s', '-W', 'ignore:Module already imported:pytest.PytestWarning'])
